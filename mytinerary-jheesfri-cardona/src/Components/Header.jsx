...
import { getCities } from "../store/actions/citiesAction";
const routes = [
...
export default function Header() {
  ...
  return (
    <header className="w-screen p-2 fixed z-50 bg-white">
      <div className="w-full grid grid-cols-3 md:grid-cols-2">
        {/* MENU HAMBURGUESA */}
        <div className="md:hidden">
          {isOpenMenu ?
            <>
              <div className="fixed top-0 left-0 z-50 bg-white w-44  border border-black rounded-lg ml-5 mt-5">
                <div className="pe-3 pt-3 flex justify-end">
                  <button onClick={() => openOrClosedMenu()}><FaBars></FaBars></button>
                </div>
                <ul className="flex flex-col justify-center items-start">
                  {routes.map((r, index) => (
                    <li key={index} className="m-1 p-1 flex">
                ...
                <div className="flex justify-evenly items-center pb-4">
                  ...
                  <img src={userNotLogin} alt="User" className="w-20" />
                </div>
              </div></>
            :
        ...
        <div className="flex justify-start items-center">
          <NavLink to="/">
            <img className="w-60 h-20 object-cover" src={logo} alt="logo" />
          </NavLink>
        ...
        <div className="flex justify-end items-center">
          ...
          <div className="flex justify-end items-center">
            <h5>User Name</h5>
            <img src={userNotLogin} alt="User" className="w-24" />
          </div>
        </div>
      </div>
    </header>
